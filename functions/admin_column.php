...
function manage_columns( $columns ) {
  ...
  $new_columns['new_post_thumb'] = __( 'Featured Image', 'tcd-horizon' );
...
function manage_post_posts_columns( $columns ) {
  ...
  foreach ( $columns as $column_name => $column_display_name ) {
    if ( isset( $columns['new_post_thumb'] ) && $column_name == 'new_post_thumb' ) {
      $new_columns['recommend_post'] = __( 'Recommend post', 'tcd-horizon' );
    }
    $new_columns[$column_name] = $column_display_name;
  }
  return $new_columns;
...
function add_column( $column_name, $post_id ) {
  ...
  switch ( $column_name ) {
    ...
    case 'recommend_post' : // おすすめ記事
      if ( get_post_meta( $post_id, 'recommend_post', true ) ) { echo '<p>' . __( 'Recommend post1', 'tcd-horizon' ) . '</p>'; }
      if ( get_post_meta( $post_id, 'recommend_post2', true ) ) { echo '<p>' . __( 'Recommend post2', 'tcd-horizon' ) . '</p>'; }
      if ( get_post_meta( $post_id, 'recommend_post3', true ) ) { echo '<p>' . __( 'Recommend post3', 'tcd-horizon' ) . '</p>'; }
      break;

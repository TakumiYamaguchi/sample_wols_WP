...
function seo_meta_box() {
  $post_types = array ( 'post', 'page', 'news', 'work');
  add_meta_box(
    'show_seo_meta_box',//ID of meta box
    __('SEO', 'tcd-horizon'),//label
...
<div class="tcd_custom_fields">
 ...
 <div class="tcd_cf_content">
  <h3 class="tcd_cf_headline"><?php _e( 'Title tag', 'tcd-horizon' ); ?></h3>
  <input type="text" name="tcd-w_meta_title" value="<?php if(!empty($seo_title)){ echo esc_attr($seo_title); }; ?>" style="width:100%" />
 </div><!-- END .content -->

 <div class="tcd_cf_content">
  <h3 class="tcd_cf_headline"><?php _e( 'Meta description tag', 'tcd-horizon' ); ?></h3>
  <p><?php printf(__('Recommended number of characters is %s.', 'tcd-horizon'), '180'); ?></p>
  ...
  $options = get_design_plus_option();
	...
	// 各タクソノミーアーカイブ
    	 ...
    	 $title = $term_meta['meta_title'];
			 ...
			 $title = sprintf( __( 'Post list for %s', 'tcd-horizon' ), $term_name );
		 }

	// 検索結果ページ
 	} elseif ( is_search() ) {
    if ( !empty( get_search_query() ) ) {
      $title = sprintf( __( 'Search result for %s', 'tcd-horizon' ), get_search_query() );
    } else {
	...
	// 日月アーカイブ
  	...
  	$title = sprintf( __( 'Archive for %s', 'tcd-horizon' ), get_the_time( __( 'F jS, Y', 'tcd-horizon' ) ) );
 	} elseif ( is_month() ) {
  	$title = sprintf( __( 'Archive for %s', 'tcd-horizon' ), get_the_time( __( 'F, Y', 'tcd-horizon') ) );
 	} elseif ( is_year() ) {
  	$title = sprintf( __( 'Archive for %s', 'tcd-horizon' ), get_the_time( __( 'Y', 'tcd-horizon') ) );
	...
	// それ以外
  	...
  	$title .= $site_title;
   	...
   	if ( $paged >= 2 || $page >= 2 ) {
     	$title = "$title $sep " . sprintf( __( 'Page %s', 'tcd-horizon' ), max( $paged, $page ) );
		}
...
function get_seo_description() {
  ...
  $site_description = get_bloginfo( 'description', 'display' );
    ...
    return sprintf( __( 'Archive for %s', 'tcd-horizon' ), get_the_time( __( 'F jS, Y', 'tcd-horizon' ) ) );
 	} elseif ( is_month() ) {
    return sprintf( __( 'Archive for %s', 'tcd-horizon' ), get_the_time( __( 'F, Y', 'tcd-horizon' ) ) );
 	} elseif ( is_year() ) {
    return sprintf( __( 'Archive for %s', 'tcd-horizon' ), get_the_time( __( 'Y', 'tcd-horizon' ) ) );
 	} elseif ( is_author() ) {
    global $wp_query;
    $curauth = $wp_query->get_queried_object();
    return sprintf( __( 'Archive for %s', 'tcd-horizon' ), esc_html( $curauth->display_name ) );
 	} elseif ( is_search() ) {
    return sprintf( __( 'Post list for %s', 'tcd-horizon' ), get_search_query() );

...
<div class="wrap">
 ...
 <h2><?php _e( 'TCD Theme Options', 'tcd-horizon' ); ?></h2> 
 ...
 <div class="updated fade">
  <p><strong><?php _e( 'Updated', 'tcd-horizon' ); ?></strong></p>
 </div>
 <?php endif; ?>
 ...
 <div id="my_theme_option" class="cf">
  ...
  <div id="my_theme_right">
   ...
   <div id="saving_data" style="display:none;"><p><?php _e('Now saving...', 'tcd-horizon'); ?></p></div>
  </div>
...
require get_template_directory() . '/admin/inc/work.php';
...
function theme_options_tools_notices() {
	...
	if ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'error' === $tools_result[1] ) {
		...
		if ( isset( $tools_result[2] ) ) {
			...
			switch( $int_import_error ) {
				...
				case 8:
					...
					break;
				case 11:
                  $message = $int_import_error . ' : ' . __( 'File has not been uploaded.', 'tcd-horizon' );
					break;
				case 12:
                  $message = $int_import_error . ' : ' . __( 'There is no file', 'tcd-horizon' );
					break;
				case 13:
                  $message = $int_import_error . ' : ' . __( 'The file extension is not .json', 'tcd-horizon' );
					break;
				case 14:
                  $message = $int_import_error . ' : ' . __( 'Failed to read the file', 'tcd-horizon' );
					break;
				case 15:
                    $message = $int_import_error . ' : ' . __( 'Json decoding failed', 'tcd-horizon' );
					break;
				default :
		...
		echo '<div class="update-message notice notice-error notice-alt"><p><strong>' . sprintf( __( 'Import error. %s', 'tcd-horizon' ), $message ) . '</strong></p></div>';

	// インポート成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings imported', 'tcd-horizon' ) . '</strong></p></div>';

	// リセット成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'reset' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings reset', 'tcd-horizon' ) . '</strong></p></div>';
	}
}
...
/**
 * horizon オプションTools デフォルト画像設定フィルター
 */
function horizon_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		...
		array(
			'media_filename' => 'image_970x1100.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_970x1100.gif',
			'theme_option_keys' => array("['contents_builder'][0]['item_list'][0]['image']","['contents_builder'][0]['item_list'][1]['image']","['contents_builder'][0]['item_list'][2]['image']")
		),
		array(
			'media_filename' => 'image_725x1100.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_725x1100.gif',
			'theme_option_keys' => array("['contents_builder'][1]['image1']","['contents_builder'][2]['image1']")
		),
		array(
			'media_filename' => 'image_725x367.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_725x367.gif',
			'theme_option_keys' => array("['contents_builder'][3]['image1']","['contents_builder'][3]['image2']","['contents_builder'][3]['image3']")
		),
		array(
			'media_filename' => 'image_1450x1100.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x1100.gif',
			'theme_option_keys' => array("archive_news_bg_image")
		),
	);
}
...
/**
 * horizon オプションTools サンプルカテゴリー
 */
function horizon_theme_options_tools_set_sample_categories( $dp_options ) {
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_categories[] = array(
			'name' => __( 'Category', 'tcd-horizon' ) . $i,
			'slug' => 'category'.$i,
			'taxonomy' => 'category',
		);
	}
...
/**
 * horizon オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function horizon_theme_options_tools_set_sample_posts( $dp_options ) {

	// ブログのアイキャッチ画像
	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_1200x776.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1200x776.gif'
	), true );

	// ワークスの画像
	$work_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_724x825.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_724x825.gif'
	), true );
	// ワークスのギャラリー画像
	$work_gallery_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_725x550.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_725x550.gif'
	), true );
	...
	for ( $i = 1; $i <= 9; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample post', 'tcd-horizon' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-horizon' ),
	...
	for ( $i = 1; $i <= 6; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample news', 'tcd-horizon' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-horizon' ),
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample work', 'tcd-horizon' ) . $i,
			'post_name' => 'sample-work' . $i,
			'post_status' => 'publish',
			'post_type' => 'work',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'thumbnail' => $work_media_id,
			'metas' => array(
				'accent_color' => '#0b5d7c',
				'single_desc' => __( 'Description will be displayed here.', 'tcd-horizon' ),
				'archive_desc' => __( 'Description will be displayed here.', 'tcd-horizon' ),
				'gallery_layout' => 'type1',
				'work_content' => array(
          array(
            'cb_content_select' => "layout1",
            'image1' => $work_gallery_media_id,
            'image2' => $work_gallery_media_id,
            'image3' => $work_gallery_media_id,
            'image4' => $work_gallery_media_id,
            'image5' => $work_gallery_media_id,
            'image6' => $work_gallery_media_id,
          )
        ),
			)
		);
...
/**
 * horizon オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function horizon_theme_options_tools_set_sample_menus( $dp_options ) {
	...
	if ( $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach( $nav_menus as $nav_menu ) {
			if ( $nav_menu->name == __( 'Sample menu', 'tcd-horizon' ) )
				return $dp_options;
		}
	}

	// サンプルメニュー作成
	$menu_id = wp_create_nav_menu( __( 'Sample menu', 'tcd-horizon' ) );
...
/**
 * horizon オプションTools サンプルウィジェット -----------------------------------------------------------------
 */
function horizon_theme_options_tools_set_sample_widgets( $dp_options ) {
	...
	$sample_widgets = array(
		...
		array(
			...
			'params' => array(
        'title1' => __('Recent post', 'tcd-horizon'),
        'post_num1' => 4,
        'post_order1' => 'date1',
        'post_type1' => 'recent_post',
        'title2' => __('Recommend post', 'tcd-horizon'),
        'post_num2' => 4,
        'post_order2' => 'date1',
		...
		array(
			...
			'params' => array(
        'search_order' => array(
          array('title' => 'category', 'show' => '1'),
          array('title' => 'archive', 'show' => '1'),
          array('title' => 'search', 'show' => '1')
        ),
        'category_headline' => __('CATEGORY', 'tcd-horizon'),
        'archive_headline' => __('ARCHIVE', 'tcd-horizon'),
        'search_headline' => __('SEARCH', 'tcd-horizon')
			)
		),
	);
 ...
 * horizon テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'horizon_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'horizon_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'horizon_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'horizon_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'horizon_theme_options_tools_set_sample_widgets', 10, 1 );
